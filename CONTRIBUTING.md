...
In the event an emergency fix is required for the release version of Chives, members of the Chives organization will create a feature branch from the current release branch `latest`.
...
sh install.sh -d
. ./activate
black . && isort benchmarks build_scripts chives tests tools *.py && mypy && flake8 benchmarks build_scripts chives tests tools *.py && pylint benchmarks build_scripts chives tests tools *.py
...
The current official testnet is testnet10. Look at `chives/util/initial_config.yaml` to see the configuration parameters
