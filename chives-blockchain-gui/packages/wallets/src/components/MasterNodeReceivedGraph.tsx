  ...
  */
  ...
  const points = [
    {
      ...
      y: BigNumber.max(0, mojoToChives(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToChives(balance).toString(), // bignumber is not supported by react
    },
  ];
  ...
  data.forEach((item) => {
    ...
    points.push({
      x: timestamp,
      y: BigNumber.max(0, mojoToChives(start)).toNumber(), // max 21,000,000 safe to number
      tooltip: mojoToChives(start).toString, // bignumber is not supported by react
    });
  });
...
export default function MasterNodeReceivedGraph(props: Props) {
...
MasterNodeReceivedGraph.defaultProps = {
  height: 150,
};
