...
import { useShowError} from '@chives/core';
import { orderBy } from 'lodash';
import { useGetWalletsQuery, useGetStrayCatsQuery, useGetCatListQuery, useAddCATTokenMutation } from '@chives/api-react';
  ...
  const list = useMemo(() => {
    ...
    let tokens = [
      ...baseWallets.map((wallet: Wallet) => ({
        ...
        name: wallet.type === WalletType.STANDARD_WALLET ? 'Chives' : wallet.name,
      })),
