...
import { useGetMasterNodeReceivedListsQuery, useGetMasterNodeReceivedListsCountQuery } from '@chives/api-react';
import type { Transaction } from '@chives/api';

export default function useWalletMasterNodeReceivedLists(
  walletId: number, 
  ...
  const { data: count, isLoading: isTransactionsCountLoading, error: transactionsCountError } = useGetMasterNodeReceivedListsCountQuery({
    walletId,
  });
  ...
  const { data: transactions, isLoading: isTransactionsLoading, error: transactionsError } = useGetMasterNodeReceivedListsQuery({
    walletId,
    start,
    end,
    sortKey,
