...
export function summaryStringsForNFTOffer(
  ...
  // TODO: Remove 1:1 NFT <--> XCC assumption
...
export function formatAmountForWalletType(
  ...
  if (walletType === WalletType.STANDARD_WALLET) {
    return mojoToChivesLocaleString(amount, locale);
...
export function offerAssetTypeForAssetId(
  ...
  if (['xcc', 'txch'].includes(assetId)) {
    assetType = OfferAsset.CHIVES;
  } else {
    const infos: OfferSummaryInfos = offerSummary.infos;
    const info: OfferSummaryAssetInfo = infos[assetId];
...
export function getNFTPriceWithoutRoyalties(
  ...
  // NFTs can only be exchanged for XCC currently
  const amountInMojos = offerAssetAmountForAssetId('xcc', summary);
  if (amountInMojos === undefined) {
    return undefined;
  }
  return mojoToChives(amountInMojos).toNumber();
}
