...
class CATWallet:
    ...
    async def create_tandem_xch_tx(
        ...
        if fee > amount_to_claim:
            chives_coins = await self.standard_wallet.select_coins(fee, min_coin_amount=min_coin_amount)
                ...
                if need_chives_transaction:
                    if fee > regular_chives_to_claim:
                        chives_tx, _ = await self.create_tandem_xch_tx(
                            fee,
                            uint64(regular_chives_to_claim),
                        ...
                        chives_tx, _ = await self.create_tandem_xch_tx(
                            fee, uint64(regular_chives_to_claim), min_coin_amount=min_coin_amount
                        )
                        innersol = self.standard_wallet.make_solution(
                            primaries=primaries,
                            coin_announcements={announcement.message},
