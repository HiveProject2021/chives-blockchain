...
from chives.util.errors import KeychainIsLocked, KeychainProxyConnectionFailure, KeychainIsEmpty, KeychainKeyNotFound
...
from chives.wallet.wallet_weight_proof_handler import get_wp_fork_point
...
class WalletNode:
    ...
    _server: Optional[ChivesServer] = None
    ...
    def get_full_node_peer(self, synced_only: bool = False) -> Optional[WSChivesConnection]:
        """
    ...
    async def fetch_puzzle_solution(
        self, height: uint32, coin: Coin, peer: Optional[WSChivesConnection] = None
    ) -> CoinSpend:
        if peer is None:
    ...
    async def fetch_children(
        self, coin_name: bytes32, fork_height: Optional[uint32] = None, peer: Optional[WSChivesConnection] = None
    ) -> List[CoinState]:
        if peer is None:
            peer = self.get_full_node_peer()
        if peer is None:
