...
class Wallet:
    ...
    async def get_puzzle_hash_by_index(self, index: int) -> bytes32:
        record: Optional[
            DerivationRecord
        ] = await self.wallet_state_manager.get_puzzle_hash_by_index(self.id(), index)
        if record is None:
            return await self.get_new_puzzlehash()
        return record.puzzle_hash
    ...
    async def create_spend_bundle_relative_chives(self, chives_amount: int, exclude: List[Coin] = []) -> SpendBundle:
        ...
        if chives_amount < 0:
            utxos = await self.select_coins(uint64(abs(chives_amount)), exclude)
        else:
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries: List[AmountWithPuzzlehash] = [
                    {"puzzlehash": newpuzhash, "amount": uint64(chives_amount), "memos": []}
                ]
