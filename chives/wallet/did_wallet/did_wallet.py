...
from chives.util.ints import uint64, uint32, uint8, uint128
...
from chives.wallet.coin_selection import select_coins
...
class DIDWallet:
    ...
    async def create_update_spend(self, fee: uint64 = uint64(0)):
        ...
        if fee > 0:
            ...
            chives_tx = await self.create_tandem_xch_tx(fee, Announcement(coin.name(), announcement_to_make))
        else:
        ...
        if chives_tx is not None and chives_tx.spend_bundle is not None:
            ...
            await self.wallet_state_manager.add_pending_transaction(chives_tx)
