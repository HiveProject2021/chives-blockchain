...
from chives.wallet.nft_wallet import nft_puzzles
from chives.wallet.nft_wallet.nft_info import NFTCoinInfo, NFTWalletInfo
from chives.wallet.nft_wallet.nft_puzzles import NFT_METADATA_UPDATER, create_ownership_layer_puzzle, get_metadata_and_phs
from chives.wallet.nft_wallet.uncurry_nft import UncurriedNFT
from chives.wallet.outer_puzzles import AssetType, construct_puzzle, match_puzzle, solve_puzzle
from chives.wallet.payment import Payment
from chives.wallet.puzzle_drivers import PuzzleInfo, Solver
from chives.wallet.puzzles.p2_delegated_puzzle_or_hidden_puzzle import (
    DEFAULT_HIDDEN_PUZZLE_HASH,
    calculate_synthetic_secret_key,
    puzzle_for_pk,
)
from chives.wallet.trading.offer import OFFER_MOD, NotarizedPayment, Offer
from chives.wallet.transaction_record import TransactionRecord
from chives.wallet.util.compute_memos import compute_memos
from chives.wallet.util.debug_spend_bundle import disassemble
...
class NFTWallet:
    ...
    async def generate_signed_transaction(
        ...
        if chives_tx is not None and chives_tx.spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, chives_tx.spend_bundle])
            chives_tx = dataclasses.replace(chives_tx, spend_bundle=None)
    ...
    async def generate_unsigned_spendbundle(
        ...
        if fee > 0:
            ...
            chives_tx = await self.create_tandem_xch_tx(fee, Announcement(nft_coin.coin.name(), announcement_to_make))
        else:
        ...
        return nft_spend_bundle, chives_tx
    ...
    async def make_nft1_offer(
        ...
        if offering_nft:  # if we are offering an NFT.
            ...
            if requested_asset_id is None:  # If we are just asking for xcc.
            ...
            if offered_asset_id is None:  # xcc offer
