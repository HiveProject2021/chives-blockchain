...
from chives.util.bech32m import bech32_decode, bech32_encode, convertbits
from chives.util.ints import uint64
from chives.wallet.outer_puzzles import (
    construct_puzzle,
    create_asset_id,
    match_puzzle,
    solve_puzzle,
...
class Offer:
    ...
    def get_pending_amounts(self) -> Dict[str, int]:
        ...
        for asset_id, coins in self.get_offered_coins().items():
            name = "xcc" if asset_id is None else asset_id.hex()
            pending_dict[name] = 0
            for coin in coins:
