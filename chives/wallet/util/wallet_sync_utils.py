...
from chives.protocols.shared_protocol import Capability
import logging
import random
...
from chives.types.blockchain_format.coin import hash_coin_ids, Coin
...
from chives.wallet.util.peer_request_cache import PeerRequestCache

log = logging.getLogger(__name__)


async def fetch_last_tx_from_peer(height: uint32, peer: WSChivesConnection) -> Optional[HeaderBlock]:
...
async def request_and_validate_removals(
    peer: WSChivesConnection, height: uint32, header_hash: bytes32, coin_name: bytes32, removals_root: bytes32
) -> bool:
...
async def request_header_blocks(
    peer: WSChivesConnection, start_height: uint32, end_height: uint32
...
async def _fetch_header_blocks_inner(
    all_peers: List[WSChivesConnection],
