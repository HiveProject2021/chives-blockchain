    ...
    with lock_and_load_config(new_root, "config.yaml") as config:
        ...
        for i in range(number_of_ph_to_search):
            ...
            for sk, _ in all_sks:
                ...
                if all_targets[-1] == config["farmer"].get("xcc_target_address") or all_targets[-2] == config[
                    "farmer"
                ].get("xcc_target_address"):
                    stop_searching_for_farmer = True
                if all_targets[-1] == config["pool"].get("xcc_target_address") or all_targets[-2] == config["pool"].get(
                    "xcc_target_address"
                ):
        ...
        if "xcc_target_address" not in config["farmer"]:
            print(
                f"Setting the xcc destination for the farmer reward (1/8 plus fees, solo and pooling)"
                f" to {all_targets[0]}"
            )
...
def chives_init(
    ...
    *,
    ...
    """
    chives_root = os.environ.get("CHIVES_ROOT", None)
    if chives_root is not None:
        print(f"CHIVES_ROOT is set to {chives_root}")
